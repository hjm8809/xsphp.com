	...
	$( '#send-btn' ).click(function(){
		...
		if(content.val() == ''){
		 ...
		 $.post(handleUrl,{username : username.val(), content : content.val()},function (data){
	 			if(data.status){	 				
	 				var str;
 					str = '<dl class="paper a1">';
					str	= str + '<dt><span class="username">'+ data.username +'</span>';
					str	= str + '<span class="num">'+ data.id +'</span></dt>';
					str = str + '<dd class="content">'+ data.content + '</dd>';
					str = str + '<dd class="bottom"><span class="time">'+ data.time + '</span>';
					str = str + '<a href="" class="close"></a></dd></dl>';
						$( '#main' ).append(str);
						$( '#close' ).click();
					
					// $( '#main' ).append(function(str){return str;});
				}else{
					alert('发布失败');
				}
